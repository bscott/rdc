...
- `window` is the focused window when the request arrived (`app: title`, truncated to 160
  ...
  was that click aimed at?" after the fact. Titles are sanitised and truncated like every other
  field, so a hostile window name cannot forge a line or rewrite your terminal. They can also
  carry document names, URLs and mail subjects; set `window_titles = false` to leave them out.

  The lookup asks the platform for the focused window directly rather than enumerating the
  desktop: `GetForegroundWindow` on Windows, `hyprctl -j activewindow` on Hyprland (in a
  blocking task with a 250 ms timeout, after which the line is written without a window),
  `NSWorkspace.frontmostApplication` plus that application's first window on macOS. Only X11 and
  other Linux compositors fall back to filtering the window list. `rdc doctor` prints how long
  the lookup takes on your machine.
