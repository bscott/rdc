...
| `user` | unset | Unix: account to drop to right after binding, when the daemon was started with root privileges. Without it `rdc serve` refuses to run with them. Set `[serve.audit].path` too, or the log lands somewhere `rdc audit` will not look. See [Running as a dedicated user](setup-linux.md#running-as-a-dedicated-user). |
