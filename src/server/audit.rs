...
impl Audit {
    pub fn disabled() -> Self {
        Self { sink: None, path: None, host: String::new(), stream: None, window_titles: false }
    }

    /// Whether handlers should look up the focused window for the log. False when the log is
    /// off entirely, so the lookup is not paid for when nothing records it.
